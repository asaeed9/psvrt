...
model20m_sr = 'SD/model20m_sr.ckpt'
...
def get_accuracy(nimgs, img_data_loc, model):
    ...
    if nimgs == -1:
        nimgs = len(train_data[:])

    train_orig_data = train_data[:nimgs]
    ...
    train_lft_labels = train_labels[:nimgs, 0]  # 2=mask_patch_2, 1=mask_patch_1
...
#        rght_labels = train_rght_labels[start_:end_]
        ...
        x_orig_batch, true_batch = next_batch(len(train), train, img_type_lbl_lft)
#        print(x_orig_batch, x_orig_batch.shape)
        mod_preds = restore_see_layer(orig=x_orig_batch, model_name=model,
        var_name='Softmax')
        mod_preds = np.where(mod_preds > 0.5, 1.0, 0.0)
#        print(mod_preds)
        cor_prd_imgs += np.count_nonzero(mod_preds[:, 0] == true_batch[:, 0])
        
        # do my stuff
        if total_imgs <= start_ + len(train):
            total_iterations += len(train)
    ...
    accuracy = cor_prd_imgs/total_imgs
    print(cor_prd_imgs, total_imgs, accuracy)
    return accuracy

if __name__ == "__main__":
    get_accuracy(50000, "../../original_images/SD", model20m_sr)
