...
m_filter_size3 = 2  # Convolution filters are 2 x 2 pixels.
...
with tf.name_scope(name_scope):
    ...
    items = tf.map_fn(lambda img_idx: tf.reshape(tf.map_fn(lambda itm_idx: tf.slice(x_image[img_idx, :, :, :],
                                                 ...
                                                 [item_size[0] * item_size[1], item_size[0], 3])
...
if __name__ == "__main__":
    ...
    optimize(num_epochs=50, save_model=True, save_name=single_patch_model, restore_model=restore_model,
             restore_name=single_patch_model)
